...
public class ChineseDocumentToSentenceProcessor implements Serializable {
  ...
  public ChineseDocumentToSentenceProcessor(String normalizationTableFile) {
    ...
    if (normalizationTableFile != null) {
      // modified by guokr --seagull
	  File file = new File(normalizationTableFile);
	  ObjectBank<String> lineIter = null;
	  if (file.exists()) {
	    lineIter = ObjectBank.getLineIterator(new File(normalizationTableFile), encoding);
	  } else {
		try {
		  URL url = new URL(normalizationTableFile);
		  Reader reader = new BufferedReader(new InputStreamReader(url.openStream()));
	      lineIter = ObjectBank.getLineIterator(reader);
		} catch (IOException e) {
		  e.printStackTrace();
		}
	  }
